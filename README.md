...
> 한국채택국제회계기준(**K-IFRS**) 기준서를 근거로, **SPC(특수목적기업)를 둘러싼 다양한 회계 쟁점**
> — 연결 판단부터 금융상품·공정가치·리스·공시까지 — 에 **이론·실무 관점**과 **명확한 출처**를
> 함께 제시하는 RAG(검색증강생성) 챗봇.
...
SPC(특수목적기업) 구조에서는 **연결 여부 판단**만이 아니라, SPC가 보유·발행한 **금융상품의 분류·측정·손상**,
**공정가치 평가**, **리스**, **별도·연결재무제표 표시**, **지분 공시** 등 다양한 회계 쟁점이 함께 얽힙니다.
이 챗봇은 그 **SPC 회계 전반**을 다루는 12개 K-IFRS 기준서를 검색해, **기준서 문단을 근거로** 답합니다.
단순 정의 나열이 아니라 **원칙을 질문의 사실관계에 적용해 결론**을 제시하고,
**참고한 기준서·페이지·문단을 출처로 보장 표시**합니다.

- **대상** — K-IFRS 적용 기업의 회계 실무자, 회계 학습자
- **도메인** — SPC 회계 전반을 포괄하는 **12개 핵심 기준서**
...
연결 판단은 대표적인 한 가지 쓰임일 뿐, SPC를 둘러싼 폭넓은 회계 쟁점을 다룹니다.

- *"우리가 설립한 SPC를 연결재무제표에 포함해야 하나요?"* — 연결·지배력 판단
- *"운용사가 보수만 받고 손익은 투자자에게 간다면 그 펀드를 연결하나요?"* — 본인·대리인
- *"SPC가 보유한 매출채권의 기대신용손실(ECL)은 어떻게 산정하나요?"* — 금융상품 손상
- *"전환사채의 부채요소와 자본요소를 어떻게 분리하나요?"* — 복합금융상품
- *"활성시장 가격이 없는 비상장주식의 공정가치는 어떻게 측정하나요?"* — 공정가치 수준
- *"판매후리스(sale and leaseback)는 어떻게 회계처리하나요?"* — 리스
...
| `ROUTING_MAX_STANDARDS` | 4 | 라우팅 기준서 상한(흔한 단어 무시 + 점수화) |
