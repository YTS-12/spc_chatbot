...
from graph.config import ROUTING_MAX_CONCEPTS, ROUTING_MAX_STANDARDS
...
def _matched_concepts(q):
    """질문 q(소문자)에서 매칭되는 개념 key 목록.

    흔한 단어 무시 + 매칭 길이로 점수화 + 약한 우연 매칭 제거 + 여유 캡.
...
def match_concepts(query: str) -> dict:
    ...
    q = query.lower()
    matched = _matched_concepts(q)

    seen, standards, theory, practice = set(), [], [], []
    include_basis = False
    frontier = list(matched)  # 점수 높은 개념이 앞쪽(우선순위)
    while frontier:
        k = frontier.pop(0)
        if k in seen:
            continue
